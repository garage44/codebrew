...
export async function unifiedVectorSearch(
    ...
    for (const result of results) {
        ...
        if (result.content_type === 'doc') {
            const doc = getDb().prepare('SELECT * FROM documentation WHERE id = ?').get(result.content_id) as
                | Documentation
                | undefined
            if (doc) {
                // Apply tag filtering if provided
                if (filters?.tags && filters.tags.length > 0) {
                    const docLabels = getDb()
                        .prepare(`
 ...
 */
export async function searchDocs(query: string, filters?: DocFilters, limit = 10): Promise<SearchResult[]> {
 ...
 * Search only tickets
 */
export async function searchTickets(query: string, filters?: DocFilters, limit = 10): Promise<TicketSearchResult[]> {
    const result = await unifiedVectorSearch(query, {
        contentType: 'ticket',
        filters,
        limit,
