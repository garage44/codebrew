...
async function generateLocalEmbedding(text: string): Promise<Float32Array> {
    try {
        ...
        const extractor = await pipeline(
            'feature-extraction',
            config.embeddings.local.model || 'Xenova/all-MiniLM-L6-v2',
        )
        ...
        throw new Error(`Local embedding generation failed: ${error instanceof Error ? error.message : String(error)}`, { cause: error })
    }
}
...
export async function generateTicketEmbedding(ticketId: string, title: string, description: string | null): Promise<void> {
    ...
    const ticket = db.prepare('SELECT repository_id FROM tickets WHERE id = ?').get(ticketId) as
