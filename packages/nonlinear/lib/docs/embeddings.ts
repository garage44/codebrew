/**
 * Embedding generation using Voyage AI (default) or local models
 */
 ...
 * Uses Voyage AI by default, falls back to local if configured
 */
...
async function storeDocEmbeddings(docId: string, chunks: Array<Chunk & {embedding: Float32Array}>): Promise<void> {
    ...
    for (const chunk of chunks) {
        ...
        try {
            ...
            db.prepare(`
                INSERT INTO vec_content (rowid, embedding, content_type, content_id, chunk_index, chunk_text, metadata)
                VALUES (?, ?, 'doc', ?, ?, ?, ?)
            `).run(
                chunkId,
                embeddingJson,
