...
export function generateNginx(domain: string): string {
    ...
    config += `# Generated by nonlinear generate-nginx --domain ${domain}\n\n`
    ...
    // HTTPS server for main domain (nonlinear)
    config += '# HTTPS server - Main domain (nonlinear)\n'
    config += 'server {\n'
    ...
    config += `        proxy_pass http://localhost:${ports['nonlinear']};\n`
    ...
    config += '    }\n\n'
    config += '    # All other routes to nonlinear\n'
    config += '    location / {\n'
    config += `        proxy_pass http://localhost:${ports['nonlinear']};\n`
    config += '        proxy_http_version 1.1;\n'
    config += '        proxy_set_header Host $host;\n'
