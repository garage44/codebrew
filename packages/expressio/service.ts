...
let bunchyConfig: {common: string; logPrefix: string; reload_ignore: string[]; separateAssets?: string[]; version: string; workspace: string} | null = null
...
void cli.usage('Usage: $0 [task]')
    ...
    , async(argv): Promise<void> => {
        ...
        for (const lang of workspace.config.languages.target) {
            ...
            // oxlint-disable-next-line no-console
            console.log(`    ${pc.green(statsTranslated[langId])} translated, ${pc.yellow(statsUntranslated[langId])} remaining`)
        }
    ...
    .command('lint', 'Lint translations', (yargs): typeof yargs => {
        ...
        const lintResult = await lintWorkspace(workspace as {
            config: {languages: {target: {id: string}[]}; source_file: string; sync: {dir: string; suggestions?: boolean}}
            i18n: Record<string, unknown>
        }, 'lint')

        if (lintResult) {
            const typedLintResult = lintResult as {create_tags: {file: string; groups: {line: number; column: number; match: string[]}[]}[]; delete_tags: {group: string; tags: {path: string[]}[]}[]}
            const maxPadding = Math.max(...typedLintResult.create_tags.map((fileGroup): number =>
    ...
    .command('start', 'Start the Expressio service', (yargs): typeof yargs => {
        ...
        if (BUN_ENV === 'development' && bunchyConfig) {
            await bunchyService(server, bunchyConfig, bunchyManager as {
                api?: {post?: (path: string, handler: (ctx: unknown, req: {data?: unknown}) => Promise<unknown>, middlewares?: unknown[]) => void}
