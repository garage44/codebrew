import {persistentState as commonPersistantState, volatileState as commonVolatileState} from '@garage44/common/lib/state'
import {mergeDeep} from '@garage44/common/lib/utils'

const persistantState = mergeDeep({...commonPersistantState} as Record<string, unknown>, {
    activeApp: null as 'expressio' | 'nonlinear' | 'pyrite' | null,
}) as typeof commonPersistantState & {activeApp: 'expressio' | 'nonlinear' | 'pyrite' | null}

const volatileState = mergeDeep(
    {
        agents: [],
        anthropic: {usage: {count: 0, limit: 1_000_000, loading: false}},
        docs: [],
        enola: {engines: {}, languages: {source: [], target: []}},
        labelDefinitions: [],
        repositories: [],
        selectedDoc: null,
        selectedLane: null,
        selectedRepository: null,
        selectedTicket: null,
        tickets: [],
        workspace: null,
        workspaces: [] as {workspace_id: string}[],
        channels: [] as {id: string; slug: string; name: string}[],
        chat: {
            activeChannelSlug: null as string | null,
            channel: '',
            channels: {main: {id: 'main', messages: [], unread: 0}},
            emoji: {active: false, list: [], lookup: {}},
            message: '',
            users: {} as Record<string, {avatar: string; username: string}>,
            width: 375,
        },
        admin: {
            authenticated: null as boolean | null,
            group: null,
            groups: [],
            permission: false,
            user: null,
            users: [],
        },
    },
    commonVolatileState,
)

export {persistantState, volatileState}
